...
# Функция, которая вызывается при получении аудио файла
async def handle_file(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(
        f"Подождите немного, я прослушаю аудиофайл {f_name} и отправлю вам текстовую "
    ...
    trance_text = 'переведенный текст с транскрипцией'
    if status_dur:
        try:
        ...
        status = False
        await update.message.reply_text(
            f"Длительность файла не должна превышать десять минут."
        )
    ...
    if status:
        ...
        await update.message.reply_text(
            f"Ловити файл с переводом. Затраченное время: {process_time} сек."
        )
        ...
        if status_dur:
            await update.message.reply_text(
                f"Возникла ошибка обработки файла. Пожалуйста, убедитесь что файл имеет правильный аудиоформат."
            )

# Функция для команды /start
async def start(update: Update, context: CallbackContext) -> None:
    await update.message.reply_text(
        'Привет! Я бот созданный для перевода песни или разговора на русский.')
...
def main():
    ...
    # Регистрируем обработчик для приема аудиофайлов
